...
grid fins (at dynamic pressure) + 2-axis TVC (pitch/yaw) + RCS (roll always,
pitch/yaw at low thrust).
"""
...
class Planner3D:
    ...
    # Final approach: lateral offset / speed weights (per s) vs time-to-go (s).
    approach_t = (0.0, 6.0, 10.0, 15.0)
    approach_pos_w = (0.3, 0.2, 0.02, 0.008)
    approach_vel_w = (0.4, 0.3, 0.03, 0.03)
    vertical_final_t = 10.0
    vertical_final_w = 0.1
    ...
    def build(self, r0, v0, m0, t_f, cfg: PlanConfig3D):
        ...
        # Horizontal distance from the pad axis, and lateral speed: weighted
        # up as touchdown approaches, so the divert is done early (high) and
        # the last seconds are flown (almost) vertically -- with the burn
        # running all the way to contact there is no final phase to clean up
        # a late lateral error.  Quadratic costs, not constraints.
        if cfg.mode != "fuel":
            tgo_k = t_f - t_k
            ...
            # in the last seconds sideways thrust (= tilt) itself is costly
            for k in range(N + 1):
                if tgo_k[k] < self.vertical_final_t:
                    f = 1.0 - tgo_k[k] / self.vertical_final_t
                    for c_ in (0, 1):
                        add_p(U(k, c_), U(k, c_), 2 * self.vertical_final_w * f * w[k])
        jw = jerk_w / dt
        for k in range(N):
            for c in range(3):
...
class Autopilot3D:
    ...
    * LANDING BURN: at ignition the touchdown time is locked and the 3-D
      ...
      until a foot touches and the engine is cut -- no separate terminal
      phase.  The last ~0.6 s are flown on the final plan with a tilt limit
      that tightens with height, so the vehicle is upright at contact.
    * Grid fins: stowed for the boost-back, deployed for the coast/entry
      and the landing burn; they steer the attitude by their real
      aerodynamic forces (see attitude_control).
    """
    ...
    touchdown_speed = 1.2
    gate_height = 0.0             # G-FOLD plan ends on the pad (base height 0) ...
    ...
    def command(self, veh, wind_now, dt):
        ...
        if self.planner.available and self.elapsed >= self.next_replan and t_go > 0.6:
        ...
        if self.plan is not None:
            ...
            max_tilt = math.radians(float(np.interp(h, *TILT_BUDGET)))
        else:
            self.phase = "FALLBACK"
    ...
    def _fly_accel(self, veh, accel, max_tilt, floor=None, align_gate=0.35):
        ...
        attitude_control(veh, d, self.k_angle, self.k_rate, self.max_rate,
                         veh.throttle < 0.25 or veh.altitude < self.low_rcs_height, hold_roll=True)
    ...
    def _pre_ignition(self, veh, r, v, m, wind_now):
        ...
        if not self._boostback_done and (self._boostback or (miss > self.boostback_miss and r[2] > 1200.0 and airspeed < 180.0)):
            ...
            if miss < self.boostback_done_miss or dvn < 0.4 or r[2] < 700.0 or miss > self._best_miss + 30.0:
                ...
                d /= np.linalg.norm(d)
                c.fins_deploy = False           # grid fins stay stowed for the boost-back
                err = math.acos(max(-1.0, min(1.0, float(np.dot(veh.axis, d)))))
        ...
        self.phase = "COAST"
        c.fins_deploy = True                    # grid fins out after the boost-back
        self._mark()
    ...
    def _ignite(self, veh, r, v, m, wind_now, t_burn):
        ...
        self._boostback = False
        self.touchdown_time = self.elapsed + max(1.5, 1.05 * t_burn) + 2.0   # + gentle final approach
...
def fin_effectiveness(veh, period=0.05):
    """Grid-fin moment model at the current flow, from the aero model:
    (M0, B) in body axes with M_fins(u) ~= M0 + B u for the (pitch, yaw,
    roll) command u in [-1, 1].  Cached for ``period`` seconds."""
    cache = getattr(veh, "_fin_eff", None)
    if cache is not None and veh.time - cache[0] < period and cache[3] == round(veh.fins_deploy, 2):
        return cache[1], cache[2]
    from aero3d import grid_fin_forces
    from rocket3d import fin_mix
    s = veh.spec
    L = veh.last
    v_rel = L.get("v_rel")
    rho = float(L.get("rho", 0.0))
    if v_rel is None or rho <= 0.0 or veh.fins_deploy < 0.05:
        M0, B = np.zeros(3), np.zeros((3, 3))
    else:
        _, zc, _, _ = veh.mass_properties()
        R = veh.R
        a_snd = float(L.get("a", 340.0))

        def mom(u):
            d = np.clip(fin_mix(u) * s.fin_max_defl, -s.fin_max_defl, s.fin_max_defl)
            _, m_w, _ = grid_fin_forces(s, R, zc, veh.omega, v_rel, rho, a_snd, d, veh.fins_deploy)
            return R.T @ m_w
        ...
        for j in range(3):
            ...
            e[j] = 0.5
            B[:, j] = (mom(e) - mom(-e)) / 1.0
    veh._fin_eff = (veh.time, M0, B, round(veh.fins_deploy, 2))
    return M0, B


# Landing-burn tilt budget vs height (m -> deg): upright at contact.
TILT_BUDGET = ([0.3, 1.5, 3.0, 12.0, 40.0], [0.6, 2.0, 5.0, 10.0, 75.0])
FIN_MIN_EFF = 1500.0   # N*m per unit fin command: below this the grid fins stay neutral
...
def attitude_control(veh, desired_axis, k_angle, k_rate, max_rate, use_rcs, hold_roll=True, roll_rate_cmd=0.0,
    ...
    Torque sources, in order: grid fins (free, strong at high dynamic
    pressure; their effectiveness is measured on the aero model at the
    current flow, so it also covers nose-first flow and Mach effects),
    engine gimbal (TVC) when the engine burns, RCS for the rest (and alone
    in near-vacuum)."""
    s = veh.spec
    c = veh.controls
    R = veh.R
    ...
    tq += 0.5 * float(min(abs(B[0, 0]), abs(B[1, 1])))
    ...
    # Cancel the known aerodynamic moment of the body (the fins are handled
    # by the allocation below, including their own restoring moment M0).
    info = veh.last.get("aero_info") or {}
    m_body = info.get("moment_body")
    if m_body is None:
        m_body = veh.last.get("aero_moment")
        m_fins_now = info.get("moment_fins")
        if m_body is not None and m_fins_now is not None:
            m_body = np.asarray(m_body) - np.asarray(m_fins_now)
    if m_body is not None:
        tau = tau - R.T @ np.asarray(m_body, float)
    tau = tau - M0
    # Grid fins first (least squares on the measured effectiveness).
    # In near-vacuum (thin air high up) the fins can give only a few hundred
    # N*m: they would flap from stop to stop for nothing.  They take over
    # progressively once they are worth a few percent of the RCS.
    fins = np.zeros(3)
    b_max = float(np.abs(B).max())
    if b_max > FIN_MIN_EFF:
        try:
            fins, *_ = np.linalg.lstsq(B, tau, rcond=1e-3)
        except np.linalg.LinAlgError:
            fins = np.zeros(3)
        fins = np.clip(fins, -1.0, 1.0) * float(np.clip((b_max - FIN_MIN_EFF) / FIN_MIN_EFF, 0.0, 1.0))
        tau = tau - B @ fins
